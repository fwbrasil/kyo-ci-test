...
/** The registry holds its instruments through `java.lang.ref.WeakReference`, and that has to be the platform's own class. A stand-in
  * under the same name, as this module once shipped for JS and Native, is linked in place of the javalib's for every binary that
  * depends on this module, and on Scala Native the javalib's `ThreadLocal` keys its entries with it: a stand-in that is not a
  * `Reference` fails the type test `ThreadLocal.Values.rehash` migrates entries through, so every `ThreadLocal` on a thread loses
  * its entry when the thread's table first grows, at ten entries.
  */

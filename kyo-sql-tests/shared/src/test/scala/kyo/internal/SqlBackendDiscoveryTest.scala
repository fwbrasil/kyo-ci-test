...
/** Pins that a backend is found at RUN time, and found by name.
  ...
  * ship, rather than one that merely happens to be unclaimed today: this suite asserts the NEGATIVE for it, so a name chosen because
  * nothing claims it yet stops testing anything the day something does.
  *

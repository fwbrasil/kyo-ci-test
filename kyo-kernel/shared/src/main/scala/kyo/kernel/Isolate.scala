    ...
    private[kyo] object internal:
        ...
        private[kernel] object Contextual extends Isolate[Any, Any, Any]:
            ...
            def isolate[A, S](state: Stack.Snapshot, v: A < S)(using Frame): (Stack.Snapshot, Stack.Snapshot, A) < S =
                ...
                // `ensureMap`, not `map`: the capture is applied as the body's value arrives. A `map` polls for a
                // stop first and would park in front of the capture with the value inside, where an abandonment
                // drops it; the value belongs to the region's done.

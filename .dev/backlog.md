...
| D3 | **`pendingPuts` and `pendingTakes` count entries of interrupted fibers.** `origin/main`'s own `pollNextLive` scaladoc says a dead put "stays in the puts queue"; the counts are plain queue sizes; the contract is "fibers currently waiting". The kernel work's interrupt tests use these counts as "the fiber is parked" barriers. Design agreed in shape (dead-entry counters: incremented when a queued waiter is interrupted, decremented when that entry leaves the queue; nothing on the fast path). Refined after reading `IOPromise`: the increment goes in the promise itself, not in `parkedTake`'s finalizer. `IOPromise` has an overridable `protected def onComplete()`, which `Http1Parser`'s `TakePromise` already uses, so the promises `takeFiber` and `putFiber` create become a channel-owned subclass that bumps the counter when it completes with an interrupt. That costs no allocation and also covers unsafe-tier callers of `putFiber`/`takeFiber`, which a safe-tier finalizer would miss (their dead entries would be decremented at exit but never incremented, and the count would drift). Exit sites decrement only for the channel's own subclass, so a caller-supplied `reuseTake` promise is never miscounted. An interrupted `putBatch` stays one dead entry until its last element is delivered. Reproduction first: park, interrupt, `assertEventually` the count is 0 | on main; affects the kernel work | not started |

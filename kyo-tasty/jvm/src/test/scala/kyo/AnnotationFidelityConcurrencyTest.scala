...
/** Concurrency fidelity for annotation fully-qualified name resolution.
  ...
  *
  * The first test's roots deliberately exclude scala-library so every `scala.*` annotation takes the unresolved path;
  * the second includes it to guard the address-based tycon resolution against the per-scope jar-pool race under
  * concurrent loads. This is the JVM-only end-to-end guard (it needs real filesystem roots and explicit concurrency
  * control); the cross-platform embedded path is covered by AnnotationFidelityTest and the deterministic
  * ClasspathOrchestratorNegIdTest.
  */
class AnnotationFidelityConcurrencyTest extends kyo.test.Test[Any]:
    ...
    "symbolsAnnotatedWith(scala.annotation.tailrec) resolves >= 1 across concurrent loads with scala-library" in {
        // Regression guard for computeFullName determinism under concurrent merge. scala-library pickles nested
        // PACKAGE nodes, and the merge writes a collapsed package's owner last-write-wins over the files that declare
        // it, in decode-arrival order, so scala.annotation can end up owned by a same-prefix scala package. The
        // query-side computeFullName must stop at the first Package like the registration side, else the owner chain
        // is re-walked and the prefix doubles ("scala.scala.annotation.tailrec") and
        // symbolsAnnotatedWith("scala.annotation.tailrec") intermittently returns 0 (the TailrecAnnotationTest failure
        // on linux-x64 JVM). Concurrency stresses the decode-arrival order; every load must resolve the full stdlib
        // @tailrec set.

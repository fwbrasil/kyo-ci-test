...
class JsonRpcHandlerTest extends JsonRpcTest:
    ...
    //
    // The offset is read from the clock and spun rather than slept: the window is a few microseconds wide, below what
    // a timer lands in, and a latch cannot stand in for it because the window closes before the handler body runs, so
    // waiting on the handler would move the close past the very race this covers.

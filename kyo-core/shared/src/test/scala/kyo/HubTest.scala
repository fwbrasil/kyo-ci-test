    ...
    "streaming" - {
        "stream delivers messages until closed" in {
            for
                h <- Hub.init[Int](4)
                l <- h.listen
                f <- Fiber.initUnscoped(l.stream().take(4).run)
                _ <- h.put(1)
                _ <- h.put(2)
                _ <- h.put(3)
                _ <- h.put(4)
        ...
        "streamFailing fails on close" in {
            for
                h     <- Hub.init[Int](4)
                l     <- h.listen
                fiber <- Fiber.initUnscoped(l.streamFailing().run)
                _     <- h.close
        ...
        "stream respects chunk size" in {
            for
                h <- Hub.init[Int](4)
                l <- h.listen
                f <- Fiber.initUnscoped(l.stream(2).mapChunk(Chunk(_)).take(2).run)
                _ <- h.putBatch(1 to 4)
                r <- f.get
        ...
        "stream handles rapid publish-consume cycles" in {
            for
                h <- Hub.init[Int](4)
                l <- h.listen
                f <- Fiber.initUnscoped(l.stream().take(1000).run)
    ...
    "concurrency" - {
        "publishers and subscribers" in {
            (for
                ...
                sub1Fiber <- Fiber.initUnscoped(
                    latch.await.andThen(
                        Async.fill(10, 10)(Abort.run(l1.take))
                    )
                )
                sub2Fiber <- Fiber.initUnscoped(
                    latch.await.andThen(
                        Async.fill(10, 10)(Abort.run(l2.take))
                    )
                )
                sub3Fiber <- Fiber.initUnscoped(
                    latch.await.andThen(
        ...
        "concurrent listeners and close" in {
            (for
                ...
                closeFiber <- Fiber.initUnscoped(latch.await.andThen(hub.close))
                _          <- latch.release
        ...
        "message ordering".onlyJvm in {
            for
                ...
                collector1 <- Fiber.initUnscoped(
                    latch.await.andThen(
                        l1.stream().take(1000).run
                    )
                )
                collector2 <- Fiber.initUnscoped(
                    latch.await.andThen(
        ...
        "backpressure with slow consumers" in {
            for
                ...
                slowConsumer <- Fiber.initUnscoped(
                    latch.await.andThen(

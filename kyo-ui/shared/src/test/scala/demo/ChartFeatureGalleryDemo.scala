...
object ChartFeatureGalleryDemo extends KyoApp:
    ...
    /** 1. Numeric color encoding + sequential gradient (cool to warm).
      ...
      */
    val sequentialColor: Svg.Root < Sync =
        Chart(readings)(
    ...
    val textAnnotations: Svg.Root < Sync =
        Chart(readings)(
    ...
    val themedPalette: Svg.Root < Sync =
        Chart(stackData)(
    ...
    /** 7. Grouped (dodged) bar with categorical colorScale: distinct per-region colors + legend. No stack.
      ...
      */
    val groupedColorScale: Svg.Root < Sync =
        Chart(saleData)(
    ...
    /** 8. Colored errorBar via colorScale: per-category whisker colors.
      ...
      */
    val coloredErrorBar: Svg.Root < Sync =
        Chart(saleData)(
    ...
    /** 10. X tick rotation + theme font: category names rotated -40 degrees in a Georgia serif font.
      ...
      */
    val rotatedTicksAndFont: Svg.Root < Sync =
        Chart(rotateData)(
    ...
    val app: UI < Sync =
        for
            svgSequentialColor     <- sequentialColor
            svgErrorBars           <- errorBars
            svgTextAnnotations     <- textAnnotations
            svgStackedArea         <- stackedArea
            svgThemedPalette       <- themedPalette
            svgAccessible          <- accessible
            svgGroupedColorScale   <- groupedColorScale
            svgColoredErrorBar     <- coloredErrorBar
            svgColoredText         <- coloredText
            svgRotatedTicksAndFont <- rotatedTicksAndFont
        yield UI.div.style(pageStyle)(
            ...
            UI.div.style(gridStyle)(
                chartCell("1. Sequential color gradient", svgSequentialColor),
                chartCell("2. Error bars (lo/hi whiskers)", svgErrorBars),
                chartCell("3. Text value annotations", svgTextAnnotations),
                chartCell("4. Stacked filled area", svgStackedArea),
                chartCell("5. Dark theme + Okabe palette", svgThemedPalette),
                chartCell("6. Accessibility (title + desc)", svgAccessible),
                chartCell("7. Grouped bar + categorical colorScale", svgGroupedColorScale),
                chartCell("8. Colored errorBar via colorScale", svgColoredErrorBar),
                chartCell("9. Colored text annotations via colorScale", svgColoredText),
                chartCell("10. X tick rotation + theme font (Georgia)", svgRotatedTicksAndFont)
            )
        )
    ...
    run {
        ...
        for
            ...
            _        <- Console.printLine(s"ChartFeatureGallery running on http://127.0.0.1:${server.port}/")
            _        <- server.await
        yield ()
        end for
    }

...
object ChartReactiveScalesDemo extends KyoApp:
    ...
    val overlayChart: Svg.Root < Sync =
        for
            (chartSvg, scales) <-
                Chart(overlayData)(
        ...
        yield
            ...
            val target =
                Svg.line
                    ...
                    .y1(targetPx)
                    .y2(targetPx)
                    .stroke(Svg.Paint.Color(Style.Color.red))
                    .strokeWidth(2.0)
    ...
    def app: UI < Async =
        for
            ...
            morphChart <-
                Chart(wave)(
                    ...
                    .size(360, 240)
                    .lower
            svgClampChart   <- clampChart
            svgOverlayChart <- overlayChart
        yield UI.div.style(pageStyle)(
            ...
            UI.div.style(gridStyle)(
                ...
                chartCell("2. Clamp (Spike=160 pinned to domain max 100)", svgClampChart),
                chartCell("3. Scale readback overlay (target rule at y=70px)", svgOverlayChart)
            )
        )
    ...
    run {
        ...
        for
            ...
            _        <- Console.printLine(s"ChartReactiveScales running on http://127.0.0.1:${server.port}/")
            _        <- server.await
        yield ()
        end for
    }

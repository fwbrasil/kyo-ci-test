...
class DocsMarkdownTest extends WebsiteTest:
    ...
    // ---- Nested fences (a 4-backtick block wrapping literal 3-backtick blocks) ----

    // The kyo-doctest README documents the DSL by wrapping literal ```scala examples in a ````markdown
    // block. A fence-length-unaware parser closes the outer block on its first inner ``` line, which
    // shattered the "Per-file defaults" section into broken prose. The outer block must stay one pre/code
    // and keep its inner fences, info-string modifiers, and the literal <!-- doctest:default --> line.
    "a 4-backtick markdown fence wrapping 3-backtick blocks renders as one verbatim code block" in {
        val source =
            "### Per-file defaults\n" +
                "\n" +
                "````markdown\n" +
        ...
        yield
            ...
            // normalizeInfoStrings must not strip the inner block's doctest modifier.
            assert(html.contains("doctest:expect=skipped"), s"the inner info-string modifier must survive: $html")
            // removeDocTestBlocks must not delete the inner literal default comment.
            assert(html.contains("doctest:default scope=inherited"), s"the inner default comment must survive: $html")
    ...
    "doctest:setup block does not leak its closing --> into prose" in {
    ...
    "leading multi-line HTML comment with text after --> keeps that text" in {
        val source =
            "<!-- a comment\n" +
    ...
    // ---- Intra-repo README.md link rewriting ----

    "[..](dir/README.md) link rewrites to the directory route" in {
    ...
    "[..](README.md) bare link rewrites to ./" in {
    ...
    "[..](README.md#anchor) keeps the fragment" in {
    ...
    "external http(s) links are left untouched by README rewriting" in {
    ...
    "heading with inline code yields plain TOC text but renders real <code>" in {
    ...
    "heading with bold/italic/link yields plain TOC text" in {
